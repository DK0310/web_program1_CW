                ...
                <div class="body">
                    ...
                    <div class="comment-actions">
                        ...
                        <?php if ($isAdmin || $isOwner): ?>
                            <button class="btn btn-small" onclick="document.getElementById('edit-form-<?= htmlspecialchars($comment['id'], ENT_QUOTES, 'UTF-8') ?>').style.display='block';this.style.display='none'">Edit</button>
                        <?php endif; ?>

                        <?php if ($isAdmin || $isOwner): ?>
                            <form action="comment_post.php" method="post" style="display:inline" onsubmit="return confirm('Delete this comment?');">
                        ...
                        <?php if ($isAdmin || $isOwner): ?>
                            <form id="edit-form-<?= htmlspecialchars($comment['id'], ENT_QUOTES, 'UTF-8') ?>" action="comment_post.php" method="post" style="display:none; margin-top:0.5em">
                                ...
                                <div style="margin-top:6px;display:flex;gap:8px;">
                                    ...
                                    <button type="button" class="btn ghost" onclick="this.parentNode.parentNode.style.display='none';this.parentNode.parentNode.previousElementSibling.querySelector('button').style.display='inline-block'">Cancel</button>
                                </div>
                            </form>
                        <?php endif; ?>
                    </div>
...
<?php if (!empty($_SESSION['user_id']) || (!empty($_SESSION['user_role']) && $_SESSION['user_role'] === 'user')): ?>
    <form action="comment_post.php" method="post" style="margin-top:16px;">
